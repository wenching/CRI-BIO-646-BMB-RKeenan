...
* Pipeline/project related files  
    - project related files (i.e., metadata & configuration file) as used in this tutorial are located under **`CRI-BIO-646/`**  
        ...
        ```
        ```{r, engine='bash', echo=FALSE}
        ls -l ../CRI-BIO-646/CRI-BIO-646.* | cut -c53-
        ```
...
```
```{r, engine='bash', echo=FALSE}
grep -A1 run.RawReadQC.FastQC.BK-AA-4_S14_L007_R355.sh result/Submit_*.bds
```
...
```
```{r, engine='bash', echo=FALSE}
grep -A1 run.alignRead.star.BK-AA-4_S14_L007_R355.sh result/Submit_*.bds
```
